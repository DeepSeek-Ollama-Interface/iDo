    ...
    <div className="h-full overflow-y-auto p-4" ref={containerRef}>
      {messages.map((msg, index) => {
            ...
            <div key={index} className="w-full mt-2 rounded-lg backdrop-blur-lg">
              ...
              {showReasoningMessageHistory && (
                <div className="mb-2 flex justify-start"> 
                  <p className="inline-block p-2 rounded-lg bg-response bg-opacity-30 text-text break-words overflow-hidden">
                    {msg.message}
                  </p>
                </div>
              )}
            ...
            if (parsedMessage.exitCode !== undefined && !(typeof parsedMessage.exitCode === 'string')) {
              ...
              return (
                <div key={index} className="flex flex-col items-start mt-2">
                  <p className="text-sm font-semibold mb-1">{formattedAuthor}</p>
                  <div className="p-2 rounded-lg bg-response text-text flex items-center gap-2">
                    🔄 <p>Operation was a loop process</p>
                  </div>
                </div>
              );
            }
        ...
        return (
          <div key={index} className={`flex flex-col ${msg.author === "user" ? "items-end" : "items-start"} mt-2`}>
            <p className="text-sm font-semibold mb-1">{formattedAuthor}</p>
            <div className={`p-2 rounded-lg ${msg.author === "user" ? "bg-primary text-text" : "bg-response text-text"} break-words overflow-hidden max-w-full`}> 
              <p className="whitespace-pre-wrap break-words overflow-hidden">{msg.message}</p>
            </div>
          </div>
        );
      })}
    ...
    <div className="overflow-auto max-h-[200px]">
      {showThinkingMessages && thinkingMessages.map((msg, index) => (
        <div key={index} className={`mb-2 ${msg.author === "user" ? "text-right" : "text-left"}`}>
          <p className="inline-block p-2 rounded-lg ${msg.author === 'user' ? 'bg-primary text-text' : 'bg-response text-text'} break-words overflow-hidden max-w-full">
            {msg.message}
          </p>
        </div>
      ))}
